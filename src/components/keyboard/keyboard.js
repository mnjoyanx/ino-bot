...
const Keyboard = ({ show = false, setShow, activeInput, type = 'text' }) => {
    ...
    const click_key = useCallback((key) => {
        ...
        switch (key.key) {
            ...
            case 'shift':
                ...
                break;
            default:
                if (activeInput.current.max == activeInput.current.value.length) return
    ...
    useKeydown({
        ...
        left: () => {
            ...
            if (activeIndex === 0) setActiveIndex(keys[type][activeRowIndex].length - 1)
        ...
        right: () => {
            ...
            if (activeIndex === keys[type][activeRowIndex].length - 1) setActiveIndex(0)
    ...
    return (
        ...
        <div className={`parent_keyboard ${show ? 'show' : ''}`} onClick={() => setShow(false)}>

            <div className={`keyboard ${uppercase ? "uppercase" : ''}`} onClick={e => e.stopPropagation()}>
                            ...
                            {rowElem.map((item, index) => {
                                return (
                                    <Key
                                        ...
                                        isActive={(activeIndex == index && activeRowIndex == rowIndex)}
                                        key={index}
                                        index={index}
                                        rowIndex={rowIndex}
                                        item={item}
