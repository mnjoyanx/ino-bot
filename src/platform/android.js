...
class Android {
  ...
  async init() {
    ...
    window.keydown = ({ keyName }) => {
      if (keyName == "back") {
        const data = { bubbles: true, cancelable: true, key: "back" };
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        let [androidVersion, versionName, versionCode] =
          window.Android.getVersion().split("|");
      ...
      const { mac, deviceId } = this.getMacAndDeviceId();
